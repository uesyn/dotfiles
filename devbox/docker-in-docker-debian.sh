#!/usr/bin/env bash
#
# Fork from: https://raw.githubusercontent.com/microsoft/vscode-dev-containers/v0.245.2/script-library/docker-in-docker-debian.sh
#
#-------------------------------------------------------------------------------------------------------------
...
dockerd_start="$(cat << 'INNEREOF'
    ...
    ( dockerd --mtu=1280 > /tmp/dockerd.log 2>&1 ) &
INNEREOF
)"
