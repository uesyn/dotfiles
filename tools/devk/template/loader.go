...
type Loader interface {
	Load(templateName, name, namespace string) (manifest.ManifestsBuilder, error)
...
func (l *loader) Load(templateName, name, namespace string) (manifest.ManifestsBuilder, error) {
...
func (l *loader) updateKustomizeFile(fSys filesys.FileSystem, kustomizationFilePath, name, namespace string) (filesys.FileSystem, error) {
	...
	kustomization.NameSuffix = "-" + name
	kustomization.Namespace = namespace
	if kustomization.CommonLabels == nil {
		kustomization.CommonLabels = make(map[string]string)
	}
	kustomization.CommonLabels[common.DevkNameLabelKey] = name
