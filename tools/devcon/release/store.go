...
func (s *store) Create(ctx context.Context, devboxName, namespace string, r *Release) error {
	logger := logr.FromContextOrDiscard(ctx).WithValues(releaseNameKey, devboxName, namespaceKey, namespace)
	ctx = logr.NewContext(ctx, logger)
	logger.V(1).Info("save release")
	if err := s.driver.Create(ctx, devboxName, namespace, r); err != nil {
		return fmt.Errorf("failed to create release: %w", err)
	}
	return nil
}

func (s *store) Get(ctx context.Context, devboxName, namespace string) (*Release, error) {
	logger := logr.FromContextOrDiscard(ctx).WithValues(releaseNameKey, devboxName, namespaceKey, namespace)
	ctx = logr.NewContext(ctx, logger)
	logger.V(1).Info("get release")
	r, err := s.driver.Get(ctx, devboxName, namespace)
	if err != nil {
...
func (s *store) Delete(ctx context.Context, devboxName, namespace string) error {
	logger := logr.FromContextOrDiscard(ctx).WithValues(releaseNameKey, devboxName, namespaceKey, namespace)
	ctx = logr.NewContext(ctx, logger)
	logger.V(1).Info("delete release")
	if err := s.driver.Delete(ctx, devboxName, namespace); err != nil {
		return fmt.Errorf("failed to delete release: %w", err)
	}
	return nil
}

func (s *store) Update(ctx context.Context, devboxName, namespace string, r *Release) error {
	logger := logr.FromContextOrDiscard(ctx).WithValues(releaseNameKey, devboxName, namespaceKey, namespace)
	ctx = logr.NewContext(ctx, logger)
	logger.V(1).Info("update release")
	if err := s.driver.Update(ctx, devboxName, namespace, r); err != nil {
